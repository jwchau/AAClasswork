...
function stepper(nums) {
    const tabu = new Array(nums.length + 1).fill(false);
    tabu[0] = true;

    for (let i = 0; i < nums.length; i++) {
        const k = nums[i];
        if (k === 0 || tabu[i] === false) continue;
        for (let j = 1; j <= k; j++) {
            tabu[i + j] = true;
        }
    }
...
//     const tabu = new Array(nums.length).fill(0);
//     tabu[0] = nums[0];
    
//     for (let i = 1; i < tabu.length; i++) {
//         let leftNeighbor = (tabu[i - 2] === undefined) ? 0 : tabu[i - 2];
//         let includeSelf =  leftNeighbor + nums[i];
//         let decludeSelf = tabu[i - 1];
//         tabu[i] = Math.max(includeSelf, decludeSelf);
