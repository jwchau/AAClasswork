...
def harrison_ford
  ...
  Actor.joins(:movies, :castings).where(name: 'Harrison Ford').where.not("castings.ord = 1").distinct.select("movies.id, movies.title")
...
def biggest_cast
  ...
  Casting.joins(:movie).group('movies.id, movies.title').order('COUNT(*) DESC').select('movies.id, movies.title').limit(3)
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).where('movies.director_id IN (?)', them).select('movies.id, movies.title')

  # Movie.joins(:actor)
  #   .where('movies.director_id = actors.id')
  #   .where('movies.director_id IN (?)', ['George Lucas'])
  #   .pluck('movies.id, movies.title')
